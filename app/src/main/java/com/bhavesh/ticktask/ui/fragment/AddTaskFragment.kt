package com.bhavesh.ticktask.ui.fragment
...
class AddTaskFragment : Fragment(R.layout.fragment_add_task) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val etRoutine = view.findViewById<EditText>(R.id.etRoutine)
        val etDecs = view.findViewById<EditText>(R.id.etDecs)
        val tvTime = view.findViewById<TextView>(R.id.tvTime)
        val tvDate = view.findViewById<TextView>(R.id.tvDate)

        val imgr = requireActivity().getSystemService(Context.INPUT_METHOD_SERVICE) as InputMethodManager
        imgr.showSoftInput(etRoutine, 0)
        etRoutine.requestFocus()

        routineDAO = context?.let { RoutineRoomDB.getDatabaseObject(it).getRoutineDAO() }!!
